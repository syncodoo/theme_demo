odoo.define('allure_backend_theme_ent.ListController', function (require) {
    ...
    ListController.include({
        ...
        willStart: function () {
            var self = this;
            return this.getSession().user_has_group('allure_backend_theme_ent.group_display_density').then(function (has_group) {
        ...
        _doUpdateSidebar: function () {
            if (this.selectedRecords.length > 0 && this.sidebar.$el.hasClass('o_drw_in')) {
                this.sidebar.$el.parents('.o_action').addClass('o_open_sidebar');
            } else {
                this.sidebar.$el.parents('.o_action').removeClass('o_open_sidebar');
            }
        },
